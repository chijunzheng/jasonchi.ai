...
- Generalized the observability agent concept into an open-source CLI and Grafana plugin ([Investigate CLI](#investigate-cli)) using OpenAI's function-calling API — 4 progressive architecture versions on the same codebase evaluated against a 40-scenario benchmark with 3 scoring methods (hit detection, LLM-as-judge, trace metrics), demonstrating that the intelligence layer pattern works independently of LLM provider and deployment scale
