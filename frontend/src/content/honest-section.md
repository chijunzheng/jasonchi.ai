...
- **Observability as an afterthought — then iterating past it.** We bolted on Langfuse after hitting production issues (should have been day-one). But even Langfuse hit a ceiling at production scale — no cross-signal correlation, no natural-language investigation. So I iterated again: self-hosted the LGTM stack and built an AI Observability Agent on top using Claude Code with eval-driven iteration (3 architecture versions, 0.882 composite on 50-question benchmark). The lesson: observability isn't a one-time setup, it evolves alongside the system it monitors
