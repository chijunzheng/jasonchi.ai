...
### Practices
- **AI-assisted development (power user):** Developed a structured workflow for building full products with coding agents — decompose features into plan documents with explicit dependency graphs, identify which features can be parallelized, then orchestrate multiple coding agents in parallel to implement them. Built custom agent skills (reusable prompt templates) for plan generation, feature tracking, and code review. This workflow powered the development of ShowMe (75K LOC), jasonchi.ai, and contributed to production system velocity at Telus. Progressed from Cursor (early adoption) to Claude Code (current primary tool) and Codex as the agentic paradigm matured
